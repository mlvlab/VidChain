...
class VideoLLaMA2DPOTrainer(DPOTrainer):
    ...
    def _save_checkpoint(self, model, trial, metrics=None):
        if getattr(self.args, 'tune_mm_mlp_adapter', False):
            ...
            if self.args.lora_enable:
                ...
                # save for acquring lora adapter parameters & trainer states: `adapter_config.json`, `adapter_model.safetensors`
                super(VideoLLaMA2DPOTrainer, self)._save_checkpoint(model, trial, metrics)
            else:
                super(VideoLLaMA2DPOTrainer, self)._save_checkpoint(model, trial, metrics)
    ...
    def _save(self, output_dir: Optional[str] = None, state_dict=None):
        if getattr(self.args, 'tune_mm_mlp_adapter', False):
            ...
            super(VideoLLaMA2DPOTrainer, self)._save(output_dir, state_dict)
