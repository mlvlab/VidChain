...
data_time_only = json.load(open("/home/jisoo/AAAI2025/VideoLLaMA2/outputs/log/generate-samples-concat-with-only-timefirst.json"))
...
final_dict  = {k:data_time_only[k] + data_c[1][k] + data_g[3][k] + data_g[0][k] + data_c[2][k] for k in ['prompt', 'chosen', 'rejected', 'vid', 'logits']}
...
with open( f'/home/jisoo/AAAI2025/VideoLLaMA2/outputs/log/generate-samples-concat-JOINT-FINAL-CAP-GENERATE-TIME.json', 'w') as f: f.write(json.dumps(final_dict, indent=2))
