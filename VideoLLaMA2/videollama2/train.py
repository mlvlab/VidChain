...
class DataArguments:
    ...
    use_dpo: bool = False
    num_sample:  Optional[int] = field(default=10)
...
def preprocess(
    ...
    modal_token = MMODAL_TOKEN_INDEX[MODAL_list[0]]
    ...
    for i, source in enumerate(sources):
        ...
        input_ids.append(tokenizer_MMODAL_token(conversation, tokenizer, modal_token, return_tensors='pt'))
        ...
        for idx, sentence in enumerate(source):
            if idx % 2 == 1:
                ...
                instruction_len = len(tokenizer_MMODAL_token(instruction, tokenizer, modal_token, return_tensors='pt'))
                conversation_len = len(tokenizer_MMODAL_token(conversation, tokenizer, modal_token, return_tensors='pt'))
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(self, data_path: str,
        ...
        for i in tqdm(((self.list_data_dict))):
            curr_id = i['id']
            if curr_id in glob_features.keys():
                i['video'] = glob_features[curr_id]
        ...
        if 'image' in sources[0]:
            ...
            if self.finetune:
                if 'meta' in sources[0]:
                    ...
                    replace_set = []
                    for k, v in sources[0]['meta']['token'].items():
                        replace_set.append((k, convert(sources[0]['meta']['duration'], v)))

            
                    for l in range(len(sources[0]['conversations'])):
                        for x1, x2 in replace_set:
                            sources[0]['conversations'][l]['value'] = sources[0]['conversations'][l]['value'].replace(x1, x2)
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        input_ids, labels, qtype = tuple([instance[key] for instance in instances]
                                  for key in ("input_ids", "labels", "qtype" ))
...
def load_lora(model, lora_path):
    ...
    if 'dpo' in lora_path:
        print('Loading dpo LoRA weights...')
    else:
