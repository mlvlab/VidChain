...
from videollama2.mm_utils import tokenizer_MMODAL_token, tokenizer_image_token, expand2square, process_video, process_image, load_lora
from videollama2.videollama2_trainer import (
    VideoLLaMA2Trainer,
    VideoLLaMA2DPOTrainer,
    maybe_zero_3, get_mm_adapter_state_maybe_zero_3,
    get_peft_state_maybe_zero_3, get_peft_state_non_lora_maybe_zero_3, 
    find_all_linear_names, safe_save_model_for_hf_trainer, find_all_linear_explicit_names
)
...
class ModelArguments:
    ...
    continue_from_model : Optional[str] = field(default=None, metadata={"help": "To train from previously trained checkpoints. E.g, further fine-tuning based on the finetuned version of the whole model."})
    continue_from_model_dpo : Optional[str] = field(default=None, metadata={"help": "To train from previously trained checkpoints. E.g, further fine-tuning based on the finetuned version of the whole model."})
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    scaleLossDC: float = 1.0
    scaleLossCE: float = 0.05
    beta: float = 0.1
    train4dpo: bool = field(default=False)
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(self, data_path: str,
        ...
        if 'videos' not in data_args.data_folder:
            if 'activitynet' in data_args.data_folder:
                print("Inside activitynet")
                self.finetune = True
                self.preloaded = True
                glob_features = glob(f'./data/activitynet/*/*.pth')
            
            if 'youcook2' in data_args.data_folder.lower():
                print("Inside YouCook2")
                self.finetune = True
                self.preloaded = True
                glob_features = glob(f'./data/YouCook2/videollama2_features/*.pth')

        else:
            if 'activitynet' in data_args.data_folder.lower():
                print("Inside activitynet raw videos")
                self.finetune = True
                glob_features = glob(f'{data_args.data_folder}/*')


            elif 'youcook2' in data_args.data_folder.lower():
                print("Inside youcook2 raw videos")
                self.finetune=True
                glob_features = glob(f'{data_args.data_folder}/*/*/*')
                

        glob_features = {i.split('/')[-1].split('.')[0] : i for i in glob_features}
        new_data_dict = []
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        data_dict = copy.deepcopy(sources[0])
        ...
        if 'image' in sources[0]:
            ...
            if self.preloaded:
                try:
                    if self.data_args.num_frames == 32:
                        video = torch.load(video_file, map_location='cpu')
                    else:
                        video = torch.load(video_file, map_location='cpu')
                        video = video.reshape(2, -1, video.shape[1], video.shape[2], video.shape[3])
                        video = video[0]
                except Exception as e:
                    traceback.print_exc()
                    backup_idx = random.randint(0, len(self.list_data_dict)-1)
...
class DPODataCollator(DPODataCollatorWithPadding):
    ...
    def tokenize_batch_element(
        ...
        chosen_data_dict = preprocess( [chosen_sources], self.tokenizer,MODAL_list=MODAL_list)
        #chosen_data_dict['attention_mask'] = chosen_data_dict["input_ids"].ne(self.tokenizer.pad_token_id)

        rejected_data_dict = preprocess([rejected_sources] ,self.tokenizer,MODAL_list=MODAL_list)
...
def train(attn_implementation=None):
    ...
    data_args.use_dpo = training_args.train4dpo
    ...
    if model_args.pretrain_model_name_or_path is not None:
        ...
        if local_rank == 0:
            print("*"*90)
            print(f"Loading pretrained model {pretrain_model_name_or_path}")
            print("*"*90)
    ...
    if training_args.lora_enable:
        ...
        continue_from_model = model_args.continue_from_model
        if continue_from_model is not None:
            # Load non-trainables + adapter
            model = load_lora(model, continue_from_model)
            
            continue_from_model_dpo = model_args.continue_from_model_dpo
            if continue_from_model_dpo is not None:
                if local_rank == 0:
                    print("*"*90)
                    print(f"Loading stage6 model {continue_from_model_dpo}")
                    print("*"*90)
                model = model.merge_and_unload()
                model = load_lora(model, continue_from_model_dpo)
    ...
    tokenizer = transformers.AutoTokenizer.from_pretrained(
        model_args.pretrain_model_name_or_path,
        cache_dir=training_args.cache_dir,
    ...
    # select a Trainer
    trainer = VideoLLaMA2DPOTrainer(model=model, ref_model = None, tokenizer=tokenizer, args=training_args, 
                    beta=training_args.beta, generate_during_eval=False, dpo_alpha=training_args.dpo_alpha, peft_config=lora_config_dpo, gamma=training_args.gamma, training_args=training_args, **data_module)
