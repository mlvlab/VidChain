...
def load_pretrained_model(model_path, model_base, model_name, args=None, load_8bit=False, load_4bit=False, device_map="auto", device="cuda", use_flash_attn=False, **kwargs):
    if 'token' in kwargs:
        token = kwargs['token']
    else:
        token = None
    ...
    if "videollama" in model_name.lower() or 'vlb' in model_name.lower():
        ...
        if 'lora' in model_name.lower():
            ...
            if 'vicuna' in model_base.lower():
                ...
                model = Videollama2MistralForCausalLM.from_pretrained(model_base, low_cpu_mem_usage=True, config=lora_cfg_pretrained, **kwargs, state_dict=state_dict)
...
def load_pretrained_extract(model_path, model_base, model_name, args=None, load_8bit=False, load_4bit=False, device_map="auto", device="cuda", use_flash_attn=False, **kwargs):
    if 'token' in kwargs:
        token = kwargs['token']
    else:
        token = None
    ...
    if "videollama" in model_name.lower() or 'vlb' in model_name.lower():
        ...
        if 'lora' in model_name.lower():
            ...
            if args.dpo_path != "":
                print('Loading dpo LoRA weights...')
                model = PeftModel.from_pretrained(model, args.dpo_path)
                # model = load_lora(model, args.dpo_path)
                print('Merging dpo LoRA weights...')
                model = model.merge_and_unload()
