...
class Videollama2MetaForCausalLM(ABC):
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        if len(input_ids) != len(X_features):
            X_features = torch.stack([X_features[0], X_features[0], X_features[1], X_features[1]], 0).cuda()
        ...
        if self.training:
            return None, attention_mask, past_key_values, new_input_embeds, new_labels, X_features.shape
        else:
