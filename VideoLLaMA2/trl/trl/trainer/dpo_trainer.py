...
class DPOTrainer(Trainer):
    ...
    def __init__(
        ...
        if not is_peft_available() and peft_config is not None:
            ...
            if isinstance(model, PeftModel):
                ...
                model = get_peft_model(model, peft_config) # create our own peft 
                model.set_adapter("default")
                
                # model.add_weighted_adapter(adapters=['default'], weights=[1.0], adapter_name='default')
                # model.set_adapter("default")
        ...
        # Explicitly the vision tower does not need to be finetuned 
        # for n,p in model.named_parameters():
        #     if 'vision_tower' in n:
        #         p.requires_grad = False
    
        # Finetune the mm_projector
        # for n,p in model.named_parameters():
    ...
    def dpo_loss(
        ...
        current_iter_rate = self.state.global_step / self.state.max_steps # TODO : multiple epochs?
        # current_iter_rate = (self.epoch + 1) * self.state.global_step / (self.state.max_steps * self.state.num_train_epochs)
