...
from vtimellm.train.dataset import DataArguments, make_supervised_dpo_data_module
...
class ModelArguments:
    ...
    stage3_path: Optional[str] = field(default=None)
    stage4_path: Optional[str] = field(default=None)
...
def train():
    ...
    if training_args.lora_enable:
        ...
        if training_args.finetuning:
            ...
            if not training_args.train4dpo:
                rank0_print("Adding LoRA adapters...")
                model = get_peft_model(model, lora_config)

        if training_args.train4dpo:
            rank0_print("*" * 90)
            rank0_print("Preparing for stage 5 (dpo training)")

            model = load_lora(model, model_args.stage4_path)
            rank0_print('Merging stage 4 LoRA weights...')
            model = model.merge_and_unload()

            rank0_print("*" * 90)
            # build new lora config for dpo
            rank0_print("Adding LoRA adapters...")
            model = get_peft_model(model, lora_config_dpo)
    ...
    data_module = make_supervised_dpo_data_module(tokenizer=tokenizer,
                                              data_args=data_args)
